...
public class LazyRatioViewer
{
	...
	{
		RandomAccessibleInterval< Pair< T, T > > paired =
				Views.interval( Views.pair( channel0, channel1 ), channel0 );

		RandomAccessibleInterval< DoubleType > ratioView =
				Converters.convert(
						paired,
						( pair, out)  -> out.set( getRatio( pair ) ),
						new DoubleType());

		return insertDimension( ratioView, imagePlusChannelDimension );
	}
	...
	{
		RandomAccessibleInterval raiWithInsertedDimension =
